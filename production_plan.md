...
- **채용 크롤러** (`scripts/monitor_seoul_jobs.mjs`): 서울 11개 교육지원청 구인 게시판(`{청}.sen.go.kr/FUS/JO/JOL11.do`)
  - ⚠️ **청마다 게시판 컬럼 구조가 완전히 다름** (서부=기관·지역·학교급·분야·직종·마감 / 중부=구분·대상·분야1·분야2·작성자 / 강남서초=직종·학교급·학교명·과목1·과목2 …). 그래서 고정 인덱스가 아니라 **헤더(`<th>`) 텍스트로 컬럼을 동적 매핑**(`buildColumnMap`)해야 함. 진단: `scripts/debug_all_headers.mjs`
  - 페이지네이션 `pageIndex` 최대 6페이지 수집(첫 페이지 10건만 보면 대량 누락 — 강서양천/강남서초는 60건+)
  - 필터: 초등·전체 학교급 + 마감 안 지남 + **블랙리스트**(영양사·행정·시설·경비·연구원 등 무관직 제외, 그 외 강사·튜터·돌봄·예체능 폭넓게 포함). 순수 유치원 제외(병설유치원은 초등 소속이라 유지)
  - 결과: 3건 → **112건** (2026-06-16 파서 재작성). **동부·북부·성동광진 3개 청은 JOL11.do에 공고를 안 올림**(GET·POST 모두 0행) — 별도 채용 시스템 추정, 후속 과제
